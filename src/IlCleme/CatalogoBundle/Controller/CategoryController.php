...
use IlCleme\CatalogoBundle\Entity\Category as Category;
use Symfony\Component\HttpFoundation\Request;
use IlCleme\CatalogoBundle\Form\CategoryType;
...
class CategoryController extends Controller
{
    ...
    public function showAction($id, Request $request)
    {
        ...
        if ( null !== $request->query->get('filterField') && $request->query->get('filterField')=='a.title' ){
            ...
            $dql = 'SELECT a FROM IlClemeCatalogoBundle:Product a JOIN a.categories c WHERE c.id = ?1 AND a.idTblLingua = ?2 AND a.title LIKE :filterValue ORDER BY a.title ASC';
            ...
            $product = $em->createQuery('SELECT a FROM IlClemeCatalogoBundle:Product a JOIN a.categories c WHERE c.id = ?1 AND a.idTblLingua = ?2 AND a.shortDescription LIKE :filterValue ');
            $product->setParameters(array(
                'filterValue' => '%'.$filterValue.'%',
                1   =>  $id,
                2   =>  $this->get('language.manager')->getSessionLanguage()
            ...
            $dql   = "SELECT a FROM IlClemeCatalogoBundle:Product a JOIN a.categories c WHERE c.id = ?1 AND a.idTblLingua = ?2 ORDER BY a.title ASC";
            $product = $em->createQuery($dql);
        ...
        return $this->render(
            'IlClemeCatalogoBundle:Welcome:showallcategoryproduct.html.twig',
